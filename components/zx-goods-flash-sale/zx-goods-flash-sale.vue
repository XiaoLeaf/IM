<template>
	<view id="zx-goods-flash-sale-root" class="zx-goods-flash-sale-root"
		...
		<view class="zx-tabs-root"
			:style="{'background-color': info.tabbar.options.bgColor, 'border-top-left-radius': info.tabbar.options.topRadius * 2 + 'rpx', 'border-top-right-radius': info.tabbar.options.topRadius * 2 + 'rpx'}">
			<view v-for="(item,index) in info.tabbar.list" :key="index" class="zx-tabs-item"
				@click="onTabsChange(index)">
				<view
					:style="{'color': index == current ? info.tabbar.options.curr.titleColor : info.tabbar.options.common.titleColor }"
					class="tabs-item-title">{{ item.title }}</view>
				<view
					:style="{'color': index == current ? info.tabbar.options.curr.descColor : info.tabbar.options.common.descColor, 'background-color': index == current ? info.tabbar.options.curr.descBgColor : info.tabbar.options.common.descBgColor}"
					class="tabs-item-subtitle">{{ item.desc }}</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			onTabsChange(index) {
				...
				this.fetchFlshSaleGoodsList(this.info.tabbar.list[index].params, this.currentInfo.list.length > 0)
			},

			/**
			 * 获取限时抢购商品列表
			 */
			fetchFlshSaleGoodsList(params, loading) {
