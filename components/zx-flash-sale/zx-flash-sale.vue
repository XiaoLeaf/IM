	...
	<view @click="onClickFlashSaleItem" class="zx-flash-sale-root" :style="{'padding': options.padding * 2 + 'rpx', 'background-color': options.bgColor, 'color': options.color, 'border-radius': options.radius * 2 + 'rpx', 'box-shadow': options.shadow ? '0 0 20rpx #eee' : ''}">
		<view class="goods-box">
			<view class="goods-img-box" :style="{'border-radius': options.imageRadius * 2 + 'rpx'}">
				<image class="goods-img" webp="true" lazy-load="true" mode="aspectFill" :src="goods.cover"></image>
			...
			<view class="goods-right">
				<view class="right-header">
					<view class="goods-title" :style="{'color': options.titleColor}">{{ goods.title }}</view>
					<view v-if="goods.subtitle" class="goods-sub-title" :style="{'color': options.subTitleColor}">{{ goods.subtitle }}</view>
					<view class="goods-price-box">
						<view class="price-content">
							<view class="real-price" :style="{'color': options.priceColor}">
								<text class="real-prefix">￥</text>
								<text>{{ goods.real_price }}</text>
								<text class="real-tail">{{ goods.real_price_ext ? goods.real_price_ext : '' }}</text>
							</view>
							<view v-if="goods.ori_price" class="ori-price">{{ goods.ori_price }}</view>
						</view>
					</view>
				</view>
				<block v-if="goods.fs_status == 1 || goods.fs_status == 3">
					<view class="right-footer">
						<view v-if="goods.fs_status == 1" class="time-box">
							<view>最后疯抢：</view>
							<u-count-down :timestamp="goods.seconds" :font-size="24" :separator-size="24" separator-color="#666" color="#666" :separator="zh"></u-count-down>
						</view>
						<view v-else-if="goods.fs_status == 3" class="time-box">{{ goods.end_time }}</view>
						<view class="slide-box">
							<view class="process-box">
								<u-line-progress :active-color="options.processColor" inactive-color="#f1f1f1" :striped-active="true" :show-percent="false" :striped="true" :height="10" :percent="goods.process"></u-line-progress>
							</view>
							<view class="percent" :style="{'color': options.processColor}">{{ goods.process }}%</view>
						</view>
					</view>
				</block>
				<block v-if="goods.fs_status == 2">
					<view class="wait-box">
						<view class="wait-time">{{ goods.start_time }}</view>
						<button @click.stop="onClickWait" class="wait-btn" :style="{'border-radius': options.subscribeBtn.radius * 2 + 'rpx!important', 'background-color': options.subscribeBtn.bgColor, 'color': options.subscribeBtn.color}">{{ options.subscribeBtn.text }}</button>
					</view>
				</block>
			</view>
		</view>
