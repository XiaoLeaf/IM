 ...
 * url: 请求链接
 * data: 请求参数
 * showLoad: 是否显示loading
 */
const serverHelper = (method, url, data, showLoad, response, error, loadTitle = '加载中...') => {
	if (showLoad) {
		uni.showLoading({
			title: loadTitle,
			mask: true
	...
	// #endif
	//操作系统
	data['sys'] = store.default.state.osName
	if (store.default.state.androidUpdate) {
		data['sys'] = '2'
	}
	//邀请人
	...
	//渠道
	let qd = uni.getStorageSync(api.qd)
	qd = qd ? qd : ''
	data['qd'] = qd
	
	data['imei'] = uni.getStorageSync(api.unionUser).toString()
	data['version'] = uni.getStorageSync(api.version).toString()
	data['token'] = sha1(hexMD5(sortObjectByKey(data)) + hexMD5(api.stStr)) //鉴权
	// console.log('接口请求参数data====>', JSON.stringify(data))

	var authorization = store.default.state.authorization
	// console.log('url====>',process.uniEnv.serverUrl)
	...
	const task = uni.request({
		url: process.uniEnv.serverUrl + url,
		method: method,
		data: data,
		timeout: 15000,
		header: headerObj,
		success: res => {
			...
			if (res.data.status === 200) {
				...
				store.default.commit('currentUserId', {currentUserId: 0})
				store.default.commit('authorization', {authorization: ''})
				store.default.commit('hasPayPwd', {hasPayPwd: false})
				store.default.commit('loginStatus', {status: false})
				openLogin()
...
function miniAppRoute(path, type = 4) {
	...
	// 	let auth = store.default.state.authorization
...
function openCmsPage(id, iden) {
	miniAppRoute('/core/pages/article/page?id=' + id + '&iden=' + iden)
}

function setClipboardData(data, toast) {
	if (!data) return
	if (!toast) {
		toast = '复制成功'
	}
...
function newUploadOssFile(host, filePath, key, policy, ossAccessKeyId, signature) {
	return new Promise((resolve, reject) => {
		uni.uploadFile({
			url: host,
			filePath: filePath,
 ...
 * 根据用户id删除当前用户信息
 */
function removeLocalUserById(id) {
 ...
 */
function openLogin() {
	store.default.commit('showLogin', {showLogin: true})
}
 ...
 */
function closeLogin() {
	store.default.commit('showLogin', {showLogin: false})
}
 ...
 */
function goGame(id) {
	if (store.default.state.loginStatus) {
		miniAppRoute('/home/game?id=' + id)
	} else {
		openLogin()
	}
}
...
function query2Json(query) {
	...
	let result = {}
	let searchStr = query.substring(query.indexOf('?') + 1)
	let queryList = searchStr.split('&')
	for (var i in queryList) {
		let tmp = queryList[i].split('=')
		result[tmp[0]] = tmp[1]
	}
	return result
}
...
module.exports = {
	...
	openCmsPage: (id, iden) => openCmsPage(id, iden),
	setClipboardData: (data, toast) => setClipboardData(data, toast),
	...
	newUploadOssFile: (host, filePath, key, policy, ossAccessKeyId, signature) => newUploadOssFile(host, filePath, key, policy, ossAccessKeyId, signature),
	...
	removeLocalUserWithIndex: (userList, index) => removeLocalUserWithIndex(userList, index),
	openLogin: openLogin,
	closeLogin: closeLogin,
	wechatBrowser: wechatBrowser,
	goGame: goGame,
	query2Json: (query) => query2Json(query),
	checkAuth: checkAuth,
